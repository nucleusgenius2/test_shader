...
function gadget:GetInfo()
  return {
    ...
    desc      = "allows to override the engine unit and feature shaders. Обработка всего что лежит в ModelMaterials.",
    author    = "jK, gajop",
...
function gadget:UnitFinished(unitID, unitDefID)
    -- Снимаем эффект когда юнит построен
    --Spring.UnitRendering.DeactivateMaterial(unitID, 3)
    --Spring.UnitRendering.DeactivateMaterial(unitID, 3,"edfBuilding") -- отключение конкретного материала
end
